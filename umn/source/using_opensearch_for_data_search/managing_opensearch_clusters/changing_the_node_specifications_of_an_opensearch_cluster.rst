:original_name: css_01_0246.html
...
If the workloads on an OpenSearch cluster's data plane change, you can scale the cluster vertically by changing its node specifications or node storage type.
...
-  Impact on OpenSearch Dashboards and Cerebro

   Changing the node storage type for a cluster will cause OpenSearch Dashboards and Cerebro to be rebuilt. During this period, they are temporarily unavailable. During a node specifications change, if OpenSearch Dashboards and Cerebro become unavailable because the node that runs them is taken offline, refresh the web page or try to log in again, and the system will reschedule them to an available node.
...
-  All mission-critical data has been backed up before a node storage type change. This is to prevent data loss. For details, see :ref:`Creating Snapshots to Back Up the Data of an OpenSearch Cluster <css_01_0298>`.
...
#. Check that all service data has replicas so that services will not be interrupted during the specifications change.
   ...
   a. In the cluster list, find the target cluster, and click **Dashboards** in the **Operation** column to log in to OpenSearch Dashboards.
   ...
   c. Run the **GET \_cat/indices?v** command in OpenSearch Dashboards.
      ...
      -  If the returned **rep** value is 0, there are no data replicas. You are advised to manually create a snapshot for the cluster before moving on to the next step. For details, see :ref:`Creating Snapshots to Back Up the Data of an OpenSearch Cluster <css_01_0298>`.
...
#. In the displayed dialog box, confirm check items, and click **OK** to start the specifications change.
   ...
   .. table:: **Table 3** Check item description
      ...
      |                                   | -  If you select **Verify index copies** and the cluster has master nodes, each index is required to have at least one replica, but there is no requirement on how many nodes the cluster must have.                                                                                                                                                    |
      +-----------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
      | Cluster status check              | During a node specifications change, the cluster status is checked by default to improve the success rate of the change and ensure data security. The nodes are changed one at a time. For each node, the system changes its specifications, restarts it, and confirms all processes have started successfully before proceeding to the next node.      |
      |                                   |                                                                                                                                                                                                                                                                                                                                                         |
      |                                   | In emergencies (for example, when a cluster is overloaded and services are faulty, which may prevent a specifications change request from being delivered), you can skip cluster status check to free up resources for cluster recovery. However, doing so may cause the cluster to become faulty and interrupt services. Exercise caution.             |
      +-----------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
