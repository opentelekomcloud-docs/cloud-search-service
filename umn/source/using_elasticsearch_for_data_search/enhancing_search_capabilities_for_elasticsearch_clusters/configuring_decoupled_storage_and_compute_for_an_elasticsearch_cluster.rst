...
As data grows exponentially, traditional monolithic storage architectures struggle to balance performance and cost efficiency. To support high-speed real-time queries while lowering the cost of storing massive quantities of data, CSS offers the storage-compute decoupling feature for Elasticsearch clusters.
...
.. figure:: /_static/images/en-us_image_0000002497835557.png
...
Decoupled storage and compute can be automated via index lifecycle management policies. For details, see :ref:`Decoupling Index Storage and Compute in an Elasticsearch Cluster Through Index Lifecycle Management <css_01_0022>`.
...
#. Check the progress of the index freezing task.
   ...
   .. table:: **Table 1** Response parameters
      ...
      +-----------------------------------+------------------------------------------------------------------------------------------------------------------+
      | indices                           | Status details of each index, as described in :ref:`Table 2 <en-us_topic_0000001965416849__table6261163344518>`. |
      +-----------------------------------+------------------------------------------------------------------------------------------------------------------+

   .. _en-us_topic_0000001965416849__table6261163344518:
   ...
   When index freezing is complete, the settings parameters will be returned, as described in :ref:`Table 3 <en-us_topic_0000001965416849__table1026263310451>`.

   .. _en-us_topic_0000001965416849__table1026263310451:
...
#. Query the list of frozen indexes.
   ...
   .. note::
      ...
      Here, only some of the key parameters for querying the frozen index list are described. For more information about the API, see `cat indices API <https://www.elastic.co/guide/en/elasticsearch/reference/7.10/cat-indices.html>`__.
...
The system index **.freeze_obs_rate-YYYY.mm.dd** records statistics on OBS real-time operation rates, helping you understand relevant trends about the OBS resources that store cold data. The default retention period of this index is 30 days.
...
To ensure acceptable performance for cold data queries (for example, when historical data is queried periodically), use a tiered storage solution. For details, see :ref:`Switching Between Hot and Cold Storage for an Elasticsearch Cluster <css_01_0079>`.
